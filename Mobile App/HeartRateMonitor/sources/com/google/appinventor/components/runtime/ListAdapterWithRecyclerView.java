...
public class ListAdapterWithRecyclerView extends RecyclerView.Adapter<RvViewHolder> implements Filterable {
    ...
    public void selectFromText(String text1) {
        for (int i = 0; i < this.itemViews.length; i++) {
            if (this.items.get(i).get(Component.LISTVIEW_KEY_MAIN_TEXT).toString() == text1) {
    ...
    class RvViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        public void onClick(View v) {
            ...
            ListAdapterWithRecyclerView.clickListener.onItemClick(position, v);
        }
    }
    ...
    public String getSelectedItems() {
        ...
        for (int i = 0; i < this.selection.length; i++) {
            if (this.selection[i].booleanValue()) {
                selectedItems = selectedItems + "," + this.items.get(i).get(Component.LISTVIEW_KEY_MAIN_TEXT).toString();
            }
        }
    ...
    public Filter getFilter() {
        return new Filter() {
            ...
            public Filter.FilterResults performFiltering(CharSequence charSequence) {
                ...
                if (filterQuery == null || filterQuery.length() == 0) {
                    ...
                    for (int i = 0; i < ListAdapterWithRecyclerView.this.items.size(); i++) {
                        YailDictionary itemDict = (YailDictionary) ListAdapterWithRecyclerView.this.items.get(i);
                        if ((itemDict.get(Component.LISTVIEW_KEY_MAIN_TEXT).toString() + " " + itemDict.get(Component.LISTVIEW_KEY_DESCRIPTION).toString()).toLowerCase().contains(filterQuery)) {
            ...
            public void publishResults(CharSequence charSequence, Filter.FilterResults filterResults) {
                ...
                if (ListAdapterWithRecyclerView.this.filterItems.size() == 0) {
                    Arrays.fill(ListAdapterWithRecyclerView.this.isVisible, Boolean.TRUE);
                    return;
                }
                for (int i = 0; i < ListAdapterWithRecyclerView.this.items.size(); i++) {
