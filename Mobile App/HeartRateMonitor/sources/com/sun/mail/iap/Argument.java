...
public class Argument {
    ...
    private void nastring(byte[] bytes, Protocol protocol, boolean doQuote) throws IOException, ProtocolException {
        ...
        for (byte b : bytes) {
            if (b == 0 || b == 13 || b == 10 || (!utf8 && (b & Ev3Constants.Opcode.TST) > Byte.MAX_VALUE)) {
                literal(bytes, protocol);
                return;
            }
            if (b == 42 || b == 37 || b == 40 || b == 41 || b == 123 || b == 34 || b == 92 || (b & Ev3Constants.Opcode.TST) <= 32 || (b & Ev3Constants.Opcode.TST) > Byte.MAX_VALUE) {
                quote = true;
                if (b == 34 || b == 92) {
                    escape = true;
                }
