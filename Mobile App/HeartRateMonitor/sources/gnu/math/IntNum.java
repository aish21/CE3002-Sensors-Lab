...
public class IntNum extends RatNum implements Externalizable {
    ...
    public void setAdd(IntNum x, int y) {
        ...
        long carry = (long) y;
        for (int i = 0; i < len; i++) {
            long carry2 = carry + (((long) x.words[i]) & UIDFolder.MAXUID);
    ...
    public static IntNum add(IntNum x, IntNum y, int k) {
        ...
        while (i < x.ival) {
            long carry2 = carry + (((long) x.words[i]) & UIDFolder.MAXUID) + y_ext;
    ...
    public long longValue() {
        ...
        return (((long) this.words[1]) << 32) + (((long) this.words[0]) & UIDFolder.MAXUID);
    }
    ...
    public static boolean negate(int[] dest, int[] src, int len) {
        ...
        for (int i = 0; i < len; i++) {
            long carry2 = carry + (((long) (src[i] ^ -1)) & UIDFolder.MAXUID);
            dest[i] = (int) carry2;
            carry = carry2 >> 32;
        }
